...
int main(int argc, char *argv[]){
  ...
  if (argc != 4){
    cerr << "Necesito tres argumentos, los ficheros que contienen las matrices y el nombre de la matriz de salida.\n";
    exit(-1);
  }
  // Tiempos
  ...
  if (rank == 0){
    ...
    for (int i = 1; i < workers; ++i){
      ...
      for (int j = 0; j < number_rows; ++j)
        MPI_Recv(&matrixf[first_row + j].front(), cols2, MPI_FLOAT, i, j + 2, MPI_COMM_WORLD, &status);
    ...
    for (int i = row_init; i < row_init + n_rows; ++i)
      for (int k = 0; k < cols1; ++k)
        for (int j = 0; j < cols2; ++j){
          ...
          matrix_parcial[i - row_init][j] += matrix1[i][k] * matrix2[k][j];
        }
